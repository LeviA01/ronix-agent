...
function buildGitPrompt(status, session) {
  ...
  if (isLearningProject() && session.purpose === "practice") {
    return [
      ...
      untrackedBlock,
      "",
      "Проведи ревью как наставник: проверь корректность, выполнение задания, ошибки, читаемость и понимание решения. Если неясно, задай вопросы. Записывай учебные свидетельства и обновляй roadmap в learning/LEARNING_DIARY.md и learning/ROADMAP.md только если практика действительно завершена.",
    ].join("\n");
  }
