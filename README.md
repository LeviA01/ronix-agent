...
## Инструкции разработки и обучения

Корневой `AGENTS.md` предназначен для правил разработки проекта. Включение
учебного режима не создаёт и не изменяет этот файл.

Ronix передаёт роль наставника вместе с каждым запросом только в сессиях
«Курс», «Теория» и «Практика», включая продолжение существующих диалогов.
Дополнительные учебные правила находятся в `learning/AGENTS.md` и действуют
только в этих сессиях. Обычные сессии и чаты не получают роль наставника;
генератор материалов получает отдельное задание на создание JSON.

Если в другом проекте остался корневой учебный `AGENTS.md` от старой версии,
замените его правилами разработки, сохранив нужные пользовательские инструкции.
Ronix не перезаписывает существующие корневые файлы. После такой замены начните
новую задачу Codex для разработки, чтобы старые инструкции не оставались в контексте.
