...
В меню приложения «Учёба» и «Разработка» открываются отдельно. В «Учёбе»
показаны учебные проекты с режимами «Курс», «Теория», «Практика» и «Прогресс»;
в «Разработке» — рабочие проекты и их сессии. Тип нового проекта определяется
разделом, из которого его добавляют. Общие «Чат», «Память» и настройки проектов
доступны как прежде, с учётом прав пользователя.

Сайдбар содержит разделы, историю сессий или учебные режимы и одну кнопку
«Меню Ronix» внизу. Через неё открываются настройки, лимиты и управление
пользователями; доступность пунктов зависит от прав и режима авторизации.
Текущий проект выбирается по названию в шапке рабочей области: меню содержит
поиск и добавление проекта через отдельный диалог. В общих чатах на этом месте
находится «Контекст · N» с выбором подключённых проектов. Контекст сохраняется
сразу, а выбор проекта и контекста доступен и при закрытом мобильном сайдбаре.

Каждый проектный раздел запоминает последний открытый проект. Переключение
сохраняет черновик и не прерывает работу агента. При обновлении старый выбранный
проект автоматически открывается в разделе своего типа; файлы и история сессий
не переносятся. Кнопка «Сделать учебным» в настройках переводит выбранный рабочий
проект в «Учёбу» и открывает его курс.
